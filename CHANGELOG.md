# Change Log

All notable changes to the "voiceplus" extension will be documented in this file.

## 0.0.1

- Added synchronized sidebar and editor chat views.
- Added Copilot model discovery, selection, and streaming responses.
- Added verified local English speech-model installation.
- Added microphone capture, silence detection, transcript review, spoken summaries, interruption, and automatic turn-taking.
- Added dynamic selectors for installed Windows speech voices and microphone devices.
- Added explicit active-selection, active-file, and browsed text-file context attachments with visible source labels.
- Added automatic active-editor context and guarded read-only workspace list, search, and file-read tools.
- Added immutable file/folder change proposals, expandable diffs, explicit approval, rejection, and conflict-aware undo.
- Added separately approved terminal batches with visible execution, captured output, safe-command auto-run policy, and Stop Task.

Check [Keep a Changelog](http://keepachangelog.com/) for recommendations on how to structure this file.

## [Unreleased]

- Initial release